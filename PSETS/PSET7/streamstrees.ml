...
(* We raise Finite_tree exception on an empty tree. *)  
let node (t : 'a tree) : 'a =
  match t with
  | lazy(Node(h,_)) -> h
...
(* We use a helper function to indent first before printing the 
   node *)   
let rec print_depth (n : int) (indent : int) (t : int tree) : unit =
  if n < 0 then ()
  else
    let rec indent_first m =
      if m > 0 then
...
let rec bfenumerate (tree_list : 'a tree list) : 'a stream =
  ...
  (* We add tl to the children of hd, so that we can do a breadth-first
     travesal in the same order *)
...
let levels (n : int) : int tree =
  let rec stream m =
    lazy (Node(m, [stream (m+1); stream (m+1)])) in
  stream n 
;;
...
let tree_nats : int tree =
  let rec stream m =
    lazy (Node(m, [stream (2*m + 1); stream (2*m + 2)])) in
  stream 0
;;
