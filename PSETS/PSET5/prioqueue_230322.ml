  ...
  struct
    ...
    let test_add () =
      ...
      unit_test ((to_string q) = "Branch (Leaf, ["^(Elt.to_string x)^"], Leaf)")
		"add 1";
      let q = add x q in
      unit_test	(to_string q = "Branch (Leaf, ["^(Elt.to_string x)^"; "^(Elt.to_string x)^"], Leaf)")
		"add 2";
      let y = Elt.generate_gt x in
      let q = add y q in
      unit_test	(to_string q = "Branch (Leaf, ["^(Elt.to_string x)^"; "^(Elt.to_string x)^"], Branch (Leaf, ["^(Elt.to_string y)^"], Leaf))")
		"add 3";
      let z = Elt.generate_lt x in
      let q = add z q in
      unit_test (to_string q = "Branch (Branch (Leaf, ["^(Elt.to_string z)^"], Leaf), ["^(Elt.to_string x)^"; "^(Elt.to_string x)^"], Branch (Leaf, ["^(Elt.to_string y)^"], Leaf))")
		"add 4";
      (* Can add further cases here *)
      ()
    ...
    let test_take () =
      ...
      unit_test	(let (a, b) = take myQ in
                 (a = x4) && (b <> empty))
		"take 1";
      unit_test (let (a, b) = take myQ in
                 (a = x4) && ((to_string b) = "Branch (Branch (Branch (Leaf, ["^(Elt.to_string x3)^"], Leaf), ["^(Elt.to_string x2)^"], Leaf), ["^(Elt.to_string x)^"], Leaf)"))
                "take 2";
      ()
