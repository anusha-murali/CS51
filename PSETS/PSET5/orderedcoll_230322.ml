...
module BinSTree (Elt : COMPARABLE)
  ...
  struct
    ...
    tree.
    In order to find x among multiple values, we filter the valus by
    comparing the incoming element with those elements that compare 
    `Equal` using List.filter. If the resulting filtered set is not
    empty, then the element is found in the tree
    ..................................................................*)
      ...
      | Branch (l, lst, r) -> 
          (match lst with
           ...
           | hd :: _ -> (match Elt.compare x hd with
                         | Less -> search x l
                         | Greater -> search x r
                         | Equal -> let lst2 = List.filter (fun y -> x = y) lst 
                                      in not (lst2 = [])))
    ...
    let rec getmin (t : tree) : elt =
      (* the minimum value is the very left most element in the tree. So,
         we recursively look for the minimum element on the left branch.
         In the case of multiple minimum elements, we need to return the
         element that was inserted first. So, we reverse the list and
         return the head. *)
      match t with
      | Leaf -> raise Empty
    ...
    let rec getmax (t : tree) : elt =
      (* the maximum value is the very right most element in the tree. So,
         we recursively look for the maximum element on the right branch.
         In the case of multiple maximum elements, we need to return the  
         element that was inserted first. So, we reverse the list and  
         return the head. *)
      match t with
      | Leaf -> raise Empty
    ...
    let test_insert () =
      ...
      unit_test (t = Branch(Leaf, [x], Leaf))
		"insert 1";
      let t = insert x t in
      unit_test	(t = Branch(Leaf, [x; x], Leaf))
		"insert 2";
      let y = Elt.generate_gt x in
      let t = insert y t in
      unit_test	(t = Branch(Leaf, [x; x], Branch(Leaf, [y], Leaf)))
		"insert 3";
      let z = Elt.generate_lt x in
      let t = insert z t in
      unit_test (t = Branch(Branch(Leaf, [z], Leaf), [x; x],
                            Branch(Leaf, [y], Leaf)))
		"insert 4";
    ...
    let test_search () =
      ...
      List.iteri (fun i value -> unit_test (search value full_tree)
					   ("search " ^ (string_of_int i)))
		 values_inserted
