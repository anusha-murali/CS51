...
(* Helper function: add two integer lists *)

let int_plus (l1 : int list) (l2 : int list) : int list =
  (plus {neg=false; coeffs=l1} {neg=false; coeffs=l2}).coeffs
;;
...
let integer_multiply b1 n =
    ...
    clean ({neg=(b1.neg || n < 0) ; coeffs=List.rev(multiply (List.rev b1.coeffs) 0)})
;;
...
let times (b1 : bignum) (b2 : bignum) : bignum =
  ...
  let m_single b1 mpl =
    let rec p_mul (lst:int list) (mplr:int list) (carry:int) =
	 ...
	 let new_carry = p_result / cBASE in
	 let result = (p_result mod cBASE) in
...
(****** NO NEED FOR THIS FUNCTION ******)
  let rec base_pow (bn:bignum) (p:int) =
    match p with
    | 0 -> bn
    | 1 -> integer_multiply bn cBASE
    | _ -> integer_multiply (base_pow bn (p-1)) cBASE 
;;
