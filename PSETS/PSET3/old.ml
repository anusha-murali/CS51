...
let equal (b1 : bignum) (b2 : bignum) : bool =
  if b1.neg <> b2.neg then
     ...
     let rec cmp_coeffs l1 l2 = 
     match (l1, l2) with
     | [], [] -> true
     | [],_ -> false
     | _,[] -> false
     | (h1::t1), (h2::t2) -> if h1 <> h2 then false
                      else cmp_coeffs t1 t2 in
     cmp_coeffs b1.coeffs b2.coeffs
;;
...
let less (b1 : bignum) (b2 : bignum) : bool =
  ...
  let length lst = List.fold_left (fun c _ -> c + 1) 0 lst in
       if (length b1.coeffs) < (length b2.coeffs) then
          true else
       if (length b1.coeffs) > (length b2.coeffs) then
          false else
       if (b1.neg = false) && (b2.neg = false) then
       let rec cmp_pos_coeffs l1 l2 =
       match (l1, l2) with
       | [], [] -> false
       | [],_ -> true
       | _,[] -> false
       ...
       | (h1::t1), (h2::t2) -> if h1 > h2 then false
                      else cmp_pos_coeffs t1 t2 in
       cmp_pos_coeffs b1.coeffs b2.coeffs else
       let rec cmp_neg_coeffs l1 l2 =
       match (l1, l2) with
       | [], [] -> false
       | [],_ -> true
       | _,[] -> false
       | [x], [y] -> if x > y then true else false
       | (h1::t1), (h2::t2) -> if h1 < h2 then false
                      else cmp_neg_coeffs t1 t2 in
       cmp_neg_coeffs b1.coeffs b2.coeffs
;;
...
(* 
   Using the equal funciton from Problem 2, we check if
   b1 and b2 are equal.  If there are not equal, we proceed
   as follows:
   1. If b1 is negative and b2 is positive, then b1 < b2
   2. If b1 is positive and b2 is negative, then b1 > b2
   3. If length of b1.coeffs is smaller than the length of 
      b2.coeffs, then b1 < b2; else b2 > b1
   4. If the above lengths are equal then: 
...
let less (b1 : bignum) (b2 : bignum) : bool =
  ...
  if (b1.neg = false) && (b2.neg = true) then false else
  let length lst = List.fold_left (fun c _ -> c + 1) 0 lst in
    if (length b1.coeffs) < (length b2.coeffs) then true else
    if (length b1.coeffs) > (length b2.coeffs) then false else
...
let to_int (b : bignum) : int option =
   let prod lst = List.fold_left (fun c _ -> c * cBASE) 1 lst in
      prod b.coeffs;;
