...
let test_tile_puzzle () : unit =
  ...
  Printf.printf("2x2 Regular DFS time:\n");
  let (dfs_path, _dfs_expanded) = call_reporting_time DFSG.solve ()  in
  flush stdout;
  assert (G.is_goal (G.execute_moves dfs_path));
...
let test_tile_puzzle () : unit =
  ...
  (* Run the solvers and report the results *)
  Printf.printf("3x3 Regular DFS time:\n");
  let (dfs_path, _dfs_expanded) = call_reporting_time DFSG.solve ()  in
  flush stdout;
  assert (G.is_goal (G.execute_moves dfs_path));
  ...
  Printf.printf("DONE TESTING 3x3 TILE PUZZLE\n");;
